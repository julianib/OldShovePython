...
# https://github.com/worldveil/deuces/pull/34/files
...
class Card:
    ...
    @staticmethod
    def get_rank_char(card_int):
        return Card.STR_RANKS[Card.get_rank_int(card_int)]
    ...
    def int_to_pretty_str(card_int):
        ...
        if color:
            if suit_int in Card.PRETTY_REDS:
                rs = Fore.BLACK + Back.WHITE + "[" + Fore.RED + str(r) + s + Fore.BLACK + "]" + Fore.WHITE + Back.BLACK
            else:
                rs = Fore.BLACK + Back.WHITE + "[" + str(r) + s + "]" + Fore.WHITE + Back.BLACK
            return rs
...
class Evaluator(object):
    ...
    def get_rank_class(self, hr):
        ...
        """
        if 0 <= hr <= LookupTable.MAX_STRAIGHT_FLUSH:
...
def convert_card_to_2char(card: Card):
    suit_int_char = {
        1: "s",
        2: "h",
        4: "d",
    ...
    return f"{Card.get_rank_char(card)}{suit_int_char[Card.get_suit_int(card)]}"


print("Import deucesrip OK")
